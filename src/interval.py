...
class App(ctk.CTk):
    ...
    def _parse_time_to_seconds(self, time_str):
        ...
        try:
            if len(parts) == 1:
                return int(parts[0])  # e.g., "300"
            elif len(parts) == 2:
                return int(parts[0]) * 60 + int(parts[1])  # e.g., "05:00"
            elif len(parts) == 3:
                return int(parts[0]) * 3600 + int(parts[1]) * 60 + int(parts[2])  # e.g., "01:00:00"
            return None
    ...
    def configuration_window(self):
        ...
        work_slider = ctk.CTkSlider(self, from_=5, to=7200, variable=self.work_time_var, width=200,
                                    command=lambda v: self._update_entry_from_slider(self.work_time_var, self.work_time_entry))
        work_slider.pack(padx=100, pady=5)
        self._update_entry_from_slider(self.work_time_var, self.work_time_entry)
        self.work_time_entry.bind("<Return>", lambda e: self._update_slider_from_entry(self.work_time_var, self.work_time_entry))
        ...
        break_slider = ctk.CTkSlider(self, from_=5, to=7200, variable=self.break_time_var, width=200,
                                     command=lambda v: self._update_entry_from_slider(self.break_time_var, self.break_time_entry))
        break_slider.pack(padx=20, pady=5)
        self._update_entry_from_slider(self.break_time_var, self.break_time_entry)
        self.break_time_entry.bind("<Return>", lambda e: self._update_slider_from_entry(self.break_time_var, self.break_time_entry))
        ...
        for week in range(18):
            for day in range(7):
                ...
                if current_date.month != last_month:
                    ...
                    month_label = ctk.CTkLabel(parent_frame, text=current_date.strftime("%b"), font=("Liberation Mono", 10))
    ...
    def update_countdown(self, on_finish_callback):
        if self.remaining_time >= 0:
            ...
            # Check if the timer label still exists before trying to configure it
